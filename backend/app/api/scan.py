...
async def create_scan(
    ...
    scan = await scan_service.get_scan(session, scan.id)
    return scan
    ...
    async def event_generator():
        ...
        while True:
            ...
            for result in scan.results:
                if result.status in ("completed", "error") and result.id not in seen_completed:
            ...
            if scan.status in ("completed", "failed"):
                yield {"event": "done", "data": json.dumps({"status": scan.status})}
                break
